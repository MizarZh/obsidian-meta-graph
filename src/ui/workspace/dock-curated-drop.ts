...
function isCancelTarget(target: Element | null): boolean {
	return Boolean(
		target?.closest(
			'.knowledge-workspace-overlay-side, .knowledge-workspace-dock-panel, .knowledge-workspace-curated-panel, .knowledge-workspace-inspector, .knowledge-workspace-connection-panel, .workspace-tab-header-container, .view-header',
		),
	);
}
